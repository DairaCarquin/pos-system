...
public interface CajaVentaRepository extends JpaRepository<CajaVentaEntity, Long> {
    ...
    @Query("SELECT v.id, v.total FROM CajaVentaEntity v WHERE v.cajaSesion.id = :cajaSesionId AND v.medioPago.nombre = 'TARJETA' AND v.estado = 'FINALIZADA'")
    List<Object[]> findVentasTarjetaPorSesion(@Param("cajaSesionId") Long cajaSesionId);

    @Query("SELECT v.id, v.total FROM CajaVentaEntity v WHERE v.cajaSesion.id = :cajaSesionId AND v.medioPago.nombre = 'EFECTIVO' AND v.estado = 'FINALIZADA'")
